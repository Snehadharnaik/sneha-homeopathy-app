...
with tabs[2]:
    ...
    sig_uploaded = st.file_uploader("Upload Signature (PNG)", type="png", key="sig")
    stamp_uploaded = st.file_uploader("Upload Stamp (PNG)", type="png", key="stamp")
    if sig_uploaded:
    ...
    cert_name = st.text_input("Patient Full Name", key="cert_name")
    cert_age = st.number_input("Age", min_value=0, max_value=120, step=1, key="cert_age")
    cert_illness = st.text_input("Illness / Reason", key="cert_illness")
    illness_start = st.date_input("Illness From Date", key="illness_start")
    illness_end = st.date_input("Illness To Date", key="illness_end")
    issue_date = st.date_input("Certificate Issue Date", value=date.today(), key="cert_issue_date")
